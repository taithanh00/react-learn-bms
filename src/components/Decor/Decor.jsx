...
const Decor = () => {
  const desiredProductIds = ["2", "3", "10", "12", "4", "5", "7", "8", "9"];

  const filteredProducts = ProductsList.filter((product) => {
    return desiredProductIds.includes(product.id);
  });

  // Iterate over the filtered products and render the ProductsCard component for each product
  filteredProducts.forEach((product) => {
    <ProductsCard key={product.id} productsmap={product} />;
  });
        ...
        <div className="container">
          <div className="shopall__container w-full h-auto">
            <div className="shopall__container__text w-full h-auto text-left grid gap-y-[80px]">
              ...
              <div className="grid grid-cols-3 gap-6 place-content-center place-items-center smallest:grid smallest:grid-cols-1 
              smallest:place-content-center smallest:place-items-center">
